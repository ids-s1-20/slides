...
## Step 2. Count number of bookings / date
...
## Step 3. Visualise number of bookings / date

.midi[
```{r out.width="65%", fig.asp = 0.4}
hotels %>%
  mutate(arrival_date = glue("{arrival_date_year} {arrival_date_month} {arrival_date_day_of_month}")) %>%
  count(arrival_date) %>%
...
.midi[
```{r out.width="65%", echo=FALSE, fig.asp = 0.4}
hotels %>%
  mutate(arrival_date = glue("{arrival_date_year} {arrival_date_month} {arrival_date_day_of_month}")) %>%
  count(arrival_date) %>%
  slice(1:7) %>%
...
## Step 3a. Visualise number of bookings / date.
...
.midi[
```{r out.width="65%", fig.asp = 0.4, message = FALSE}
hotels %>%
  mutate(arrival_date = ymd(glue("{arrival_date_year} {arrival_date_month} {arrival_date_day_of_month}"))) %>% 
  count(arrival_date) %>%
  ggplot(aes(x = arrival_date, y = n, group = 1)) +
