...
email_pred %>%
  mutate(
    spam      = if_else(spam == 1, "Actual spam", "Actual not spam"),
    spam_pred = if_else(.pred_1 > cutoff_prob, "Predict spam", "Predict not spam")
    ) %>%
  count(spam, spam_pred) %>%
  pivot_wider(names_from = spam_pred, values_from = n) %>%
  kable(col.names = c("", "Predict not spam", "Predict spam"))
```
]
]
