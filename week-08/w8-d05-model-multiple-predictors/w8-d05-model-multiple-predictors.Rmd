...
book_fit_main <- linear_reg() %>%
  set_engine("lm") %>%
  fit(weight ~ volume + cover, data = allbacks)
book_fit_main_aug <- augment(book_fit_main$fit)

book_fit_int <- linear_reg() %>%
  set_engine("lm") %>%
  fit(weight ~ volume + cover + volume*cover, data = allbacks)
book_fit_int_aug <- augment(book_fit_int$fit)
...
```{r}
glance(book_fit_main)$r.squared
glance(book_fit_int)$r.squared
```
...
.pull-right[
```{r}
glance(book_fit_main)$adj.r.squared
glance(book_fit_int)$adj.r.squared
```
]
...
```{r}
glance(book_fit_int)$r.squared > glance(m_main)$r.squared 
```

- Is R-sq adj. higher for int model?

```{r}
glance(book_fit_int)$adj.r.squared > glance(m_main)$adj.r.squared
```
]
]
